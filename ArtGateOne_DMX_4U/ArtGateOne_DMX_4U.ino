/*
  ArtGateOne DMX 4 universe - test - for arduino mega
*/
...
void setup()
{
  ...
  ArduinoDmx1.set_control_pin(-1);  // Arduino output pin for MAX485 input/output control (connect to MAX485-1 pins 2-3)(-1 not used)
  ...
  ArduinoDmx2.set_control_pin(-1);  // Arduino output pin for MAX485 input/output control (connect to MAX485-1 pins 2-3)(-1 not used)
  ArduinoDmx2.set_tx_address(1);    // set rx1 start address
  ArduinoDmx2.set_tx_channels(512); // 2 to 2048!! channels in DMX1000K (512 in standard mode) See lib_dmx.h  *** new *** EXPERIMENTAL
  ArduinoDmx2.init_tx(DMX512);      // starts universe 1 as tx, standard DMX 512 - See lib_dmx.h, now support for DMX faster modes (DMX 1000K)

  ArduinoDmx3.set_control_pin(-1);  // Arduino output pin for MAX485 input/output control (connect to MAX485-1 pins 2-3)(-1 not used)
  ArduinoDmx3.set_tx_address(1);    // set rx1 start address
  ArduinoDmx3.set_tx_channels(512); // 2 to 2048!! channels in DMX1000K (512 in standard mode) See lib_dmx.h  *** new *** EXPERIMENTAL
  ArduinoDmx3.init_tx(DMX512);      // starts universe 1 as tx, standard DMX 512 - See lib_dmx.h, now support for DMX faster modes (DMX 1000K)
...
void loop()
{
      ...
      if (client.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          if (strwww[0] == 71 && strwww[5] == 32) {
            ...
            client.println(F("<meta name=\"description\" content=\"ArtGateOne DMX 4U v.1.0 setup page.\">"));
            ...
            client.println(F("<h2>ArtGateOne 4U Setup</h2>"));
  ...
  if (packetSize == 530) {
    ...
    else if (packetBuffer[15] == intN && packetBuffer[14] == intUniverse + 1) { //check artnet net & universe (sub/uni)
      for (int i = 0; i <= 511; i++) {
        Udp.read(packetBuffer, 1);
        ArduinoDmx1.TxBuffer[i] = packetBuffer[0];
      }
    }
    else if (packetBuffer[15] == intN && packetBuffer[14] == intUniverse + 2) { //check artnet net & universe (sub/uni)
      for (int i = 0; i <= 511; i++) {
        Udp.read(packetBuffer, 1);
        ArduinoDmx2.TxBuffer[i] = packetBuffer[0];
      }
    }
    else if (packetBuffer[15] == intN && packetBuffer[14] == intUniverse + 3) { //check artnet net & universe (sub/uni)
      for (int i = 0; i <= 511; i++) {
        Udp.read(packetBuffer, 1);
        ArduinoDmx3.TxBuffer[i] = packetBuffer[0];
      }
    }
...
void makeArtPoolReply() {
  ...
  ArtPoolReply[59] = byte('4');  // 4
  ArtPoolReply[60] = byte('U');  // U
  ...
  ArtPoolReply[173] = 0x04; // NumPorts Lo
  ArtPoolReply[174] = 0x80; // Port 0 Type
  ArtPoolReply[175] = 0x80; // Port 1 Type
  ArtPoolReply[176] = 0x80; // Port 2 Type
  ArtPoolReply[177] = 0x80; // Port 3 Type
  ...
  ArtPoolReply[191] = intU + 1; // SwOut 1
  ArtPoolReply[192] = intU + 2; // SwOut 2
  ArtPoolReply[193] = intU + 3; // SwOut 3
...
void displaydata() {
  ...
  oled.print(" , ");
  oled.print(intUniverse + 1);
  oled.print(" , ");
  oled.print(intUniverse + 2);
  oled.print(" , ");
  oled.print(intUniverse + 3);
